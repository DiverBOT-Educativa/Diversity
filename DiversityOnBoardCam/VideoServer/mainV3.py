...
#Para poder usar este script tanto en linux como en windows, verificamos antes
if os.name!='nt':    #estamos en linux
    ...
    logging.basicConfig(
    filename="/home/curiosity/Documentos/ServidorVideo.log",
    level=logging.DEBUG,
    format="%(asctime)s:%(levelname)s:%(message)s"
    )

else:                 #estamos en windows
...
def accept_client(client_socket, addr):
    ...
    logging.debug('Cliente {} connected'.format(connected_client_ip))
    ...
    while True:
        ...
        try:
            if frame_result != False:
                numTXFrames += 1
                print('numTXFrames{}'.format(numTXFrames))
                data = pickle.dumps(current_frame,0)
                client_socket.sendall(struct.pack("=L", len(data)) + data)

        except socket.error as e: # Handle client disconnect
            if e.errno == socket.errno.ECONNRESET:
                logging.debug('Cliente {} disconnected'.format(connected_client_ip))
                break
            else:
...
def process_camera(camera, encode_param):
    ...
    while camera.isOpened():
        """Las imagenes capturadas por la cámara se almacenan en un buffer
            por lo que si no se procesan de forma rápida, cada vez hay más
            delay en la imagen.
        """
        ...
        if ret != False:
            ...
            frameGetted.set()
            """ Mientras esperamos a que se envie el frame, leemos continuamente la camara
                para mantener el buffer vacio
            """
            while frameGetted.is_set():     #Esperamos a que desactiven el flag desde algún thread
                ret, frame = camera.read()
        else:
            print("No frame")   

frameGetted = threading.Event()
